...
void Obj::check_collision() {
    bool is_coll = false;
    float xo, yo, lx, ly, angle, f;
    float len = 0;
    float temp_speed_x, temp_speed_y;
...
void create_obj(int xid, int xtype, int xx, int xy, int xmass, int xvx, int xvy, int xax, int xay, int xfx, int xfy, int xmoveable, sf::Color xcolor) {
    Obj obj;
    obj.id = xid;
    obj.type = xtype;
    obj.color = xcolor;
...
int main()
{
    create_obj(g_id, 0, W/2, H/2, Mass_zero, 0, 0, 0, 0, 0, 0, 0, sf::Color::Green);
    ...
    while (window.isOpen())
    {
        ...
        while (window.pollEvent(event))
        {
            ...
            if (sf::Mouse::isButtonPressed(sf::Mouse::Left))
            {
                ...
                sf::Color color_rand = sf::Color::White;
                color_rand.r = rand();
                color_rand.g = rand();
                color_rand.b = rand();

                create_obj(g_id, 0, position.x,position.y, 1, 0, 0, 0, 0, 0, 0, 1,color_rand );
            }
       ...
       // text.setPosition(50, 50);
        ...
        for (int i = 0; i < g_id; i++) {
            ...
            if (g_mas[i].type == 0) {
                ...
                circle.setFillColor(g_mas[i].color);
                window.draw(circle);
