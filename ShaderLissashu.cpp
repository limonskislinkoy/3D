...
int main()
{
    ...
    */
    ...
    objects[8] = 1.0;// mat
    objects[9] = 1.0;// coef_prelom
    objects[10] = 0.0;// prosrachnost
    ...
    objects[LEN_OBJ * k_id + 8] = 0.0;// mat
    objects[LEN_OBJ * k_id + 9] = 1.0;// zerkalo




    k_id = 10;
    objects[LEN_OBJ * k_id + 0] = 1.0;//type                          # 11 #  CIRCLE LIGHT NEBO
    objects[LEN_OBJ * k_id + 1] = -5.0;//x
    objects[LEN_OBJ * k_id + 2] = 0.0;//y
    objects[LEN_OBJ * k_id + 3] = 10.0;//z
    objects[LEN_OBJ * k_id + 4] = 1.0;//ra

    objects[LEN_OBJ * k_id + 5] = 0.305;//r
    objects[LEN_OBJ * k_id + 6] = 0.870;//g  
    objects[LEN_OBJ * k_id + 7] = 0.956;//b
    objects[LEN_OBJ * k_id + 8] = 0.0;// mat
    objects[LEN_OBJ * k_id + 9] = 1.0;// zerkalo


    k_id = 11;
    objects[LEN_OBJ * k_id + 0] = 1.0;//type                          # 1 #  CIRCLE green
    objects[LEN_OBJ * k_id + 1] = 0.0;//x
    objects[LEN_OBJ * k_id + 2] = 0.0;//y
    objects[LEN_OBJ * k_id + 3] = 1.0;//z
    objects[LEN_OBJ * k_id + 4] = 1.0;//ra

    objects[LEN_OBJ * k_id + 5] = 0.0;//r
    objects[LEN_OBJ * k_id + 6] = 1.0;//g
    objects[LEN_OBJ * k_id + 7] = 0.0;//b

    objects[LEN_OBJ * k_id + 8] = 0.5;// mat
    objects[LEN_OBJ * k_id + 9] = 1.0;// coef_prelom


    k_id = 12;
    objects[LEN_OBJ * k_id + 0] = 1.0;//type                          # 1 #  CIRCLE blue
    objects[LEN_OBJ * k_id + 1] = 0.0;//x
    objects[LEN_OBJ * k_id + 2] = 3.0;//y
    objects[LEN_OBJ * k_id + 3] = 1.0;//z
    objects[LEN_OBJ * k_id + 4] = 1.0;//ra

    objects[LEN_OBJ * k_id + 5] = 0.0;//r
    objects[LEN_OBJ * k_id + 6] = 0.0;//g
    objects[LEN_OBJ * k_id + 7] = 1.0;//b
    ...
    objects[LEN_OBJ * k_id + 8] = 0.7; // mat
    objects[LEN_OBJ * k_id + 9] = 0.0; // prosrachnost


    k_id = 1;
    objects[LEN_OBJ * k_id + 0] = 1.0;//type            # 2 # Circle Sun

    objects[LEN_OBJ * k_id + 1] = 2.0;// x
    objects[LEN_OBJ * k_id + 2] = 2.0;// y
    objects[LEN_OBJ * k_id + 3] = 1.0; // z
    ...
    k_id = 2;
    objects[LEN_OBJ * k_id + 0] = 1.0;//type            # 3 # Circle Nebo

    objects[LEN_OBJ * k_id + 1] = 0.0;// x
    objects[LEN_OBJ * k_id + 2] = 0.0;// y
    objects[LEN_OBJ * k_id + 3] = 0.0; // z

    objects[LEN_OBJ * k_id + 4] = 1000.0;//ra

    objects[LEN_OBJ * k_id + 5] = 0.7;//r   0.266
    objects[LEN_OBJ * k_id + 6] = 0.7; //g  0.658
    objects[LEN_OBJ * k_id + 7] = 0.7; //b  0.933
    ...
    objects[LEN_OBJ * k_id + 1] = 3.0;//point 1 x
    objects[LEN_OBJ * k_id + 2] = -3.0;//point 1 y
    objects[LEN_OBJ * k_id + 3] = 5.0; //point 1 z

    objects[LEN_OBJ * k_id + 4] = 3.0;//point 2 x
    objects[LEN_OBJ * k_id + 5] = 3.0;//point 2 y
    objects[LEN_OBJ * k_id + 6] = 0.0; //point 2 z

    objects[LEN_OBJ * k_id + 7] = 3.0;//point 3 x
    objects[LEN_OBJ * k_id + 8] = -3.0;//point 3 y
    objects[LEN_OBJ * k_id + 9] = 0.0; //point 3 z
    ...
    */
    ...
    while (window.isOpen()) {
        ...
        if (wasd[0] && !wasd[2]) {
            player_pos.x += speed *cos(player_dir_angle.x);
            player_pos.y += speed *sin(player_dir_angle.x);
        }

        if (wasd[1] && !wasd[3]) {
            player_pos.x += speed * sin(player_dir_angle.x);
            player_pos.y -= speed * cos(player_dir_angle.x);
        }
